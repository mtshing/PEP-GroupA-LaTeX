...
\aleq{
    \begin{cases}
    \dvv{S}{x}\eval_{x=x_0} = 0 
    & \qty(\substack{\text{Entropy is the max.}\\\text{when }x=x_0}\\\Rightarrow \text{Slope }=0)\\
    %
    \dvv[2]{S}{x}\eval_{x=x_0} <0
    & \qty(\substack{\text{Slope is decreasing}\\\text{at the maximum}})
    \end{cases}
}
...
\aleq{
    Z = 
    \bcase{
        \sum_{\substack{\text{all possible}\\x_i}}W(x_i) 
        = \sum_{\substack{\text{all possible}\\x_i}}e^{\frac{S(x_i)}{k}}
        & \text{if }x \text{ can only take discrete values}
        %
        \int_{-\infty}^\infty W(x) \dd{x} 

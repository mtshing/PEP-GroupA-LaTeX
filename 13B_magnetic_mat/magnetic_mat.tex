...
\begin{overview}
    ...
    \end{itemize}
    \begin{center}
        \red{{{(If linear magnetic material is all you need, feel free to skip all vector calculus)}}}
    \end{center}
\end{overview}
...
Atoms's interactions with B-field are mostly from electrons.
Although the \cul[red]{true explanations require quantum mechanics}, 
We can discuss 2 semi-classical theories of how they act as magnetic dipoles:
\begin{itemize}
    \item \ul{Orbital motion} - Electrons orbit around nucleus, 
    similar to a constant current loop.


    \insertFig{orbital motion}

    \item \ul{Spin} - Electrons carry a fundamental quantum property called \bf{spin},
    making them intrinsically a ``point'' magnetic dipole.
    Spin cannot be described fully by any classical picture.

    \insertFig{electron spin}

    % text side by side to the figure
    Electron's spin is usually depicted as a rotating charged arrow, 
    although electron is a \it{point} particle.

\end{itemize}

Since electrons are the souce of B-field,
...
\insertFig{view at short distance vs long distance}
% i can see the arrows/loop, holding magnifying glass
% what? they are at the same pos, i cannot tell them apart, arm on waist
...
In terms of responses, atoms as magnetic dipole can interact with external B-field in opposite ways,
depend on which of the two electrons' contributions dorminates:

\begin{center}
    \begin{tabular}{>{\centering\arraybackslash}m{0.17\linewidth}|
        >{\centering\arraybackslash}m{0.36\linewidth}|
        >{\centering\arraybackslash}m{0.36\linewidth}}
        & 
        \bf{Diamagnetism} % insert graphic
        & 
        \bf{Paramagnetism} % insert graphic
        \\[1ex]
        %
        \hline
        \makecell{\phantom{\scriptsize aaa}\\[-2ex] Dominated\\Behavior}
        &
        ...
        &
        \makecell{\phantom{\scriptsize aaa}\\[-1.7ex] 
            Most materials, where electrons\\with opposite spin all pair up\\such that spin effect is cancelled
        }
        &
        \makecell{\phantom{\scriptsize aaa}\\[-0.4ex]
            Materials with unusual bondings\\that contain unpaired electrons.
        }
        
    \end{tabular}
\end{center}

Although the formal explanation requires quantum mechanics,
we may still briefly explain why they lead to opposite behavior with classical pictures:

\begin{itemize}
    \item Electron are points objects - they can flip their spins easily to align with B-field 
    \item Orbital motion are real current loops - induced B-field is generated to oppose the external B-field,
    which is equivalent to creating dipoles opposite to the external B-field.
\end{itemize}

\insertFig{classical pic of dia/para}
...
\begin{notation}[Side Note:]
    ...
    \begin{itemize}
        ...
        \item Ferromagnetism is a collective behavior of many neighbouring atoms. 
    ...
    \insertFig{natural -> near magnetic -> expanding domain}

    When placed under an external B-field, 
    the domains which aligns with the field expand to merge other domains, 
    causing a strong and unify magnetization.
...
Recall that magnetic response of atoms is contributed by their electrons -
by their orbital motion and spin.
Depends on the contribution of each behavior and the material structure,
dipole density in the material can vary by position.
The final dipole alignment might not always align with the external B-field! 

\insertFig{many dipoles under external B-field, not fully align}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \ul{Total B-field} - Can be measured through \it{Hall effect}.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item $\mu_r$ = \bf{Relative permeability}. A pure number. $\bcase{<1 &\text{ for diamagnetism}\\>1 &\text{ for paramagnetism}}$.
...
\begin{itemize}
    ...
    \item $\chi_m$ is more oftenly tabulated in material handbooks and also used in theories.
    This is because most material has very small $\chi_m$ (e.g. $10^{-5}$ for aluminum).
    Tabulating $\mu_r = 1+\chi_m$ will waste a lot space to print the `0's.
\end{itemize}

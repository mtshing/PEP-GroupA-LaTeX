...
Substituting the power law potential, it is found that the integral can be resolved into simple functions only when $\alpha = 2, -1$ or $-2$.  
...
\aleq{
    ...
    & \qty(\substack{\text{Completing}\\[0.2em]\text{square}}) \\
    %
...
\aleq{
    ...
    %
    \qty(x + \frac{\epsilon r_0}{1-\epsilon^2})^2 + \frac{y^2}{1-\epsilon^2} &= \frac{r_0^2}{1-\epsilon^2} + \frac{\epsilon^2r_0^2}{(1-\epsilon^2)^2} \\
    &= \qty(\frac{r_0}{1-\epsilon^2})^2 \\
    %
    \frac{\qty(x + \frac{\epsilon r_0}{1-\epsilon^2})^2}{\qty(\frac{r_0}{1-\epsilon^2})^2} + \frac{y^2}{\qty(\frac{r_0}{\sqrt{1-\epsilon^2}})^2} &= 1 \\[0.5em]
    %
    \Rightarrow \quad 
    \Aboxed{
...
Kepler's \nth{3} Law only applies to circular and elliptical orbit, because the concept of "period" only applies to closed orbits. 
...
If we go back to the standard proof in section \ref{standard proof} stop at step 1, 
...
\aleq{
    ...
    \int \dd{t} &= \sqrt{\frac{m}{2}} \int \frac{r\dd{r}}{\sqrt{Er^2 \pm kr - \frac{L^2}{2m}}}
}

Again, the $\pm$ sign is for: \red{$+$ve = attractive force, $-$ve = repulsive force.} Integrate by trigonometric substitution gives

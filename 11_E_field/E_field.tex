...
Promoting Coulomb's law from point charge to a distribution of charge
is straightforward by
\aleq{
    \ \ \qty(\mstack{\text{Point}\\\text{Charge}}) \ =\ Q 
    \quad\ \xRightarrow{\phantom{000}}\quad\ \ 
    \rho \dd{\tau} \ =\ \qty(\mstack{\text{Charge}\\\text{Density}})\qty(\mstack{\text{Unit}\\\text{Volume}})
}

It is then a sum of all E-field contribution from every infinitestimal volumes:
\aleq{
    \vvec{E} = \inv{4\pi\epsilon_0} \frac{Q}{r^2} \hhat{r}
    \quad\xRightarrow{\phantom{000}}\quad
    \int \dd{\vvec{E}} 
    = \int_{\substack{\text{whole}\\\text{space}}} \inv{4\pi\epsilon_0}  \frac{\rho\dd{\tau}}{r^2}\hhat{r}
}

And electrical potential follows:
\aleq{
    V = \inv{4\pi\epsilon_0} \frac{Q}{r}
    \quad\xRightarrow{\phantom{000}}\quad
    \int \dd{V} 
    = \int_{\substack{\text{whole}\\\text{space}}} \inv{4\pi\epsilon_0}  \frac{\rho\dd{\tau}}{r}
}
...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \aleq{
            V = \int_a^b \inv{4\pi\epsilon_0} \frac{\frac{Q}{L}\dd{x}}{\sqrt{z^2+x^2}}
        }
        ...
        \aleq{
            E_z = \int_a^b \inv{4\pi\epsilon_0} \frac{\frac{Q}{L}\dd{x}}{z^2+x^2} \cdot \frac{z}{\sqrt{z^2+x^2}}
        }

        Similarly for the horizontal component ($x$):
        \aleq{
            E_x = \int_a^b \inv{4\pi\epsilon_0} \frac{\frac{Q}{L}\dd{x}}{z^2+x^2} \cdot \frac{x}{\sqrt{z^2+x^2}}
        }
...
\aleq{
    \qty(\mstack{\text{Flux of}\\\text{electric field}\\\text{on a surface}})
    \ =\ \oiint \vvec{E}\cdot \dd{\vvec{s}}
    \ =\ \frac{Q}{\epsilon_0}
    \ =\ \frac{(\text{Charge enclosed})}{(\text{Some constant})}
}
...
We can first define a normal vector $\vvec{s}$ on the surface.
By convention, this $\vvec{s}$ 
\begin{itemize}
    \item has a magnitude equal to the surface's area.
...
\begin{itemize}
    ...
    they are more or less in the same direction $\Rightarrow \vvec{F}$ is pointing outward!

    \item If $\vvec{F}\cdot\vvec{s}<0$,
    they are more or less in opposite direction $\Rightarrow \vvec{F}$ is pointing inward!
\end{itemize}

Finally, we construct the definition of "flux" $\Phi$:
\aleq{
    \Aboxed{
        \Phi = \sum_{\substack{\text{all small grids }i\\\text{in the surface}}} \vvec{F}_i \cdot \vvec{s}_i
        \quad\xrightarrow{\phantom{000}}\quad
...
\aleq{
    \tkn{div}{\cul[red]{\div}}\ (\cdot) 
    \ \defeq\  \pdvv{x}(\cdot) + \pdvv{y}(\cdot) + \pdvv{z} (\cdot)
    \ \defeq\ \tkn{div2}{\cul[blue]{\text{div}}}\ (\cdot)
}
...
and will return a scalar (number).
\aleq{
    \div \vvec{F} 
    &= \bmat{\pdvv{x} & \pdvv{y} & \pdvv{z}}\bmat{F_x\\F_y\\F_z}\\
    &= \pdvv{F_x}{x} + \pdvv{F_y}{y} + \pdvv{F_z}{z}\\
    &= (\text{A number})
}

Computing divergence of a vector field is equivalent to 
\bf{calculating the flux over an infinitestimal small but closed surface.}

%%%%%%%%%%%%%%
\subsubsection{Geometrical Interpretation}

To visualize, we can draw in infinitestimal cube around a point $(x,y,z)$.
Although a cube has 6 faces, 
because of symmetry in all 3 directions,
it is enough to just analyze by the 2 faces that are parallel to the $x$-$y$ plane.

\insertFig{infinitestmal cube + 2 face}

\begin{itemize}
    \item Normal direction of these 2 faces = $\hhat{z}$ \ $\Rightarrow$\ 
    The surface normal vector $=\blue{\hhat{z} \dd{x}\dd{y}}$.

    \item Center point of top surface = $\qty(x,y,z+\frac{\dd{z}}{2})$ 
    \ $\Rightarrow$\ The field pokes through it $=\red{\vvec{F}\qty(x,y,z+\frac{\dd{z}}{2})}$.

    \item Center point of bottom surface = $\qty(x,y,z-\frac{\dd{z}}{2})$ 
    \ $\Rightarrow$\ The field pokes through it $=\red{\vvec{F}\qty(x,y,z-\frac{\dd{z}}{2})}$.
...
\aleq{
    \qty(\mstack{\text{Total Out-flux through}\\[0.5ex]\text{surfaces}\parallel\text{x-y plane}}) 
    &= \qty(\mstack{\text{Out-flux through}\\[0.5ex]\text{top face}}) - 
        \qty(\mstack{\text{In-flux through}\\[0.5ex]\text{bottom face}})\\[1em]
    %
    \dd{\Phi_{xy}} &= 
    \textstyle (\vvec{F}\qty(x,y,z+\frac{\dd{z}}{2}) \cdot \hhat{z} \dd{x}\dd{y}) - 
        (\vvec{F}\qty(x,y,z-\frac{\dd{z}}{2}) \cdot \hhat{z} \dd{x}\dd{y})\\[1em]
    %
    &= \qty(\frac{\vvec{F}\qty(x,y,z+\frac{\dd{z}}{2}) - \vvec{F}\qty(x,y,z-\frac{\dd{z}}{2})}{\blue{\dd{z}}}) 
        \cdot \hhat{z} \dd{x}\dd{y}\blue{\dd{z}}\\[1ex]
    %
    &= \cut[red]{\cbox[red]{\qty(\frac{\vvec{F}\qty(x,y,\cbox[red]{\textstyle z+\frac{\dd{z}}{2}}) 
        - \vvec{F}\qty(x,y,\cbox[red]{\textstyle z-\frac{\dd{z}}{2}})}{\cbox[red]{\dd{z}}})}}{\text{This is exactly partial z}}
        \cdot \hhat{z} \dd{x}\dd{y}\dd{z}\\[1ex]
    %
    &= \pdvv{z}\cul[green]{\vvec{F}(x,y,z)\cdot \hhat{z}} \dd{x}\dd{y}\dd{z}\\[1em]
    %
    &= \pdvv{z}\tkn{div_dot}{\cul[green]{F_z}}(x,y,z)\ (\dd{x}\dd{y}\dd{z})\\[1em]
    %
...
\aleq{
    ...
    \quad \sim \sum_{\text{All volumes}} \frac{(\text{Flux})}{(\text{Volume})}\times \qty(\text{Volume})
}
...
Gauss's law is purely an \cul[red]{observation} about the relation between E-field and charges:
\aleq{
    \Aboxed{

...
\begin{itemize}
    ...
    \aleq{
        ...
        &= -\tkn{init_r2}{\cul[blue]{R}} \sin \qty(\frac{qB_z}{m}t + \tkn{phase2}{\cul[blue]{\phi}}) 
            - \frac{E_x}{B_z}t + \tkn{init_y}{\cul[blue]{y_0}}
    }
...
\begin{notation}[Note:]
    ...
    %Imagine if the EMF produces a magnetic flux that is in the \cul[red]{same direction} of the change.


    %That means the charges inside the ring will be accelerated to infinite speed. 
    %Completely violating energy conservation.

    Beware that induced EMF only SLOW DOWN the change in the magnetic flux.
    It does not affect the final value of the flux. 
    \begin{center}
        \begin{minipage}{0.45\linewidth}
            \centering
            \includegraphics[width=0.7\textwidth]{inductance1}\\
            Without induced EMF,\\
            flux changes arruptly.
        \end{minipage}
        \quad\vline\quad
        \begin{minipage}{0.45\linewidth}
            \centering
            \includegraphics[width=0.7\textwidth]{inductance2}\\
            With induced EMF,
            flux reaches the same final value,
            but more gently.
        \end{minipage}
    \end{center}

    Also, because induced EMF slows down the change in flux by introducing an opposite flux,
    which will again create another EMF. 
    This is the effect of \bf{self-induction}. 
    The EMF as a function of time has to be determined by solving ODEs.
...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item Direction of the EMF shall be determined by Lenz's law. 
        It depends on how the current varies.
        For example, if $\dv{I(t)}{t}>0$,
        \begin{itemize}
            \item Magnetic flux increasing in the into-paper direction.
            \item To oppose the change, it requires an out-of-paper direction flux. 
            \item To produce such flux, EMF must be in anti-clockwise direction.
        \end{itemize}

...
\aleq{
    \text{state} = (\vvec{x}_1, \vvec{v}_1, \vvec{x}_2, \vvec{v}_2,..., \vvec{x}_N, \vvec{v}_N)
}
...
\begin{center}
    \red{This is the irreversibility!}
\end{center}
...
Consider a closed system of 2 boxes $A$ and $B$,
which are in a macrostate with a multiplicity $W_A$ and $W_B$ respectively.
\begin{itemize}
    \item If we look at them independently, 
    \aleq{
        S_\text{total} = S_A + S_B = \phi(W_A) + \phi(W_B)
    }

    \insertFig{separate box}

    \item If we look at them as a combined system,
    because calculating multiplicity is just counting the number of combination,
    \aleq{
        W_\text{total} &= W_A \times W_B\\
...
This is how we arrive at the Boltzmann entropy formula:
\aleq{
    \Aboxed{
...
\begin{enumerate}
    ...
    \aleq{
        ...
        \tkn{S_n}{\cul[red]{n}}\tkn{S_R}{\cul[green]{R}} &= \tkn{S_N}{\cul[blue]{N}}k\\[2em]
    ...
    \addArrow[yellow]{S_NA}{(7ex,0)}{\scriptsize Avogadro's number}{(2ex,0.5ex)}{(5ex,0)}
\end{enumerate}
...
\cul[red]{The chain of atoms would like to contract under heat!}
...
\insertFig{smooth curve}

The temperature scale defined by entropy is kind of "weird"...

\insertFig{low energy -> high energy}

In this sense, \red{temperature is no long about hot or cold anymore.}

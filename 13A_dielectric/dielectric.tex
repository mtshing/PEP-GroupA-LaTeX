...
\begin{overview}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Describe external field: Free charges, electric displacement field
        \end{itemize}
    \end{itemize}
    \begin{center}
...
This is the standard model of electric dipole.
Same calculations can be found in most textbooks.
...
\begin{enumerate}
    \item The total potential contributions from the two charges at position $\vvec{r}$ is
    \aleq{
        V(\vvec{r}) 
        ...
        &= \frac{q}{4\pi\epsilon_0} 
            ...
            - \frac{1}{\tkn{cosLaw2}{\cul[blue]{\sqrt{\norm{\vvec{r}}^2 + \qty|\frac{\vvec{d}}{2}|^2 + 2\norm{\vvec{r}}\qty|\frac{\vvec{d}}{2}|\cos\theta}}}})\\[3ex]
            %
        &= \frac{q}{4\pi\epsilon_0 \norm{\vvec{r}}}
            \qty(\inv{\sqrt{1 + \inv{4}\frac{\norm{\vvec{d}}^2}{\norm{\vvec{r}}^2} - \frac{\norm{\vvec{d}}}{\norm{\vvec{r}}}\cos\theta}}
            - \inv{\sqrt{1 + \inv{4}\frac{\norm{\vvec{d}}^2}{\norm{\vvec{r}}^2} + \frac{\norm{\vvec{d}}}{\norm{\vvec{r}}}\cos\theta}})
    }
    \addBentArrow[blue]{cosLaw1}{(3ex,-2ex)}{These are just cosine law}{(0,-3ex)}{(12ex,0.5ex)}
    \addBentArrow[blue]{cosLaw2}{(-3ex,-2ex)}{}{(0,-3ex)}
    ...
    \aleq{
        V(\vvec{r}) &= \frac{q}{4\pi\epsilon_0 \norm{\vvec{r}}}
            \qty[\qty(1 + \half \cdot \frac{\norm{\vvec{d}}}{\norm{\vvec{r}}}\cos\theta)
            - \qty(1 - \half \cdot \frac{\norm{\vvec{d}}}{\norm{\vvec{r}}}\cos\theta)]\\[1ex]
        &= \frac{q}{4\pi\epsilon_0 \norm{\vvec{r}}}
            \cdot \frac{\norm{\vvec{d}}}{\norm{\vvec{r}}}\cos\theta
    }
    ...
    \aleq{
        ...
        &= \frac{q}{4\pi\epsilon_0}
            \cdot \frac{\norm{\vvec{d}}}{\norm{\vvec{r}}^2} 
            \cdot \frac{\vvec{d}\cdot\vvec{r}}{\norm{\vvec{d}}\norm{\vvec{r}}}\\
        &= \frac{1}{4\pi\epsilon_0} \cdot \frac{q\vvec{d}}{\norm{\vvec{r}}^2} 
            \cdot \cul[blue]{\tkn{unitVecR}{\frac{\vvec{r}}{\norm{\vvec{r}}}}}
    }
    \addBentArrow[blue]{unitVecR}{(2ex,-2ex)}{\scriptsize Unit vector of $\vvec{r}$}{(0,-3ex)}{(5.5ex,0.7ex)}
    
    We usually use symbol $\vvec{p}\defeq q\vvec{d}$ to denote \bf{electric dipole moment},
    the unit source of electric dipole. 
    Finally we reach the standard formula of electric potential from a dipole:
    \aleq{
        \Aboxed{
...
\begin{itemize}
    ...
    Magnitude of potential depends on the charge on dipole AND the angle (dot product) between $\vvec{p}$ and $\hhat{r}$.
...
\begin{notation}[The boring derivation:]
    \begin{enumerate}
        \item Turn $V(\vvec{r})$ into full x-y-z form by 
        ...
        \item Calculating the $x$ term in gradient, $\hhat{x}\pdvv{x}(\cdots)$, we have
        ...
        \addBentArrow[red]{dipoleE5}{(7ex,-2ex)}{\scriptsize Take out magnitude to become unit vector}{(0,-1ex)}{(14ex,0.6ex)}
        \aleq{
            -\grad V(\vvec{r}) &= -\inv{4\pi\epsilon_0} \qty(\hhat{x}\pdvv{x}+\hhat{y}\pdvv{y}+\hhat{z}\pdvv{z})
                \qty[\frac{1}{(x^2 + y^2 + z^2)^{\frac{3}{2}}}(p_x x + p_y y + p_z z)]\\[1ex]
            ...
            &= \inv{4\pi\epsilon_0} \inv{\norm{\vvec{r}}^5} \qty[3(\vvec{p}\cdot\red{\hhat{r}})\tkm{dipoleE5}\red{\hhat{r}} \cdot \red{\norm{\vvec{r}}^2} - \vvec{p}\norm{\vvec{r}}^2]\\[1ex]
            \Aboxed{
                \vvec{E}(\vvec{r}) &= \inv{4\pi\epsilon_0} \inv{\norm{\vvec{r}}^3} \qty[3(\vvec{p}\cdot\hhat{r})\hhat{r} - \vvec{p}]
            }
        }
    \end{enumerate}
...
In general, all dielectric material are made of many tiny electric dipoles.
To analyze the material's electrostatic properties, 
...
\aleq{
    \cub[blue]{V(\vvec{r}) = \inv{4\pi\epsilon_0} \frac{\vvec{p}_i }{\norm{\vvec{r}}^2} \cdot \qty(\frac{\vvec{r}}{\norm{\vvec{r}}})}
...
Here we introduce \cul[red]{two quantities that are used to describe dipole arrangements in material}.
\begin{itemize}
...
we can replace 
\begin{itemize}
    \item Summmation of all dipoles \ $\Rightarrow$\  Volume integral over the whole object.
    \item Discrete dipoles source \ $\Rightarrow$\ A vector distribution called \bf{polarization field} $\vvec{P}(\vvec{r})$.
\end{itemize}
\aleq{
    \Aboxed{
...
The polarization field $\vvec{P}(\vvec{r})$ can also be interpreted as \bf{dipole density} 
...
Ultimately, dipoles are just pairs of charges.
If dipoles are aligned non-uniformly in a material,
...
So bound charge distribution is another way to describe the dipole arrangement in the material.
...
\begin{notation}[The boring derivation:]
    \begin{enumerate}
        ...
        \aleq{
            ...
            \frac{\vvec{P}(\vvec{r}')}{\norm{\vvec{r}-\vvec{r}'}^2} \cdot \qty(\frac{\vvec{r}-\vvec{r}'}{\norm{\vvec{r}-\vvec{r}'}}) \dd[3]{\vvec{r}'} \\[1ex]
            &= \inv{4\pi\epsilon_0} {\iiint}
                \cus[blue]{\vvec{P}(\vvec{r}') \cdot \grad \qty(\inv{\norm{\vvec{r}-\vvec{r}'}})}{\scriptsize \vvec{V}\,\cdot\,\grad f} \dd[3]{\vvec{r}'}\\[0.5ex]
            &= \inv{4\pi\epsilon_0} {\iiint}
                \cus[blue]{\div \qty(\frac{\vvec{P}(\vvec{r}')}{\norm{\vvec{r}-\vvec{r}'}})}{\scriptsize \div(f\vvec{V})} 
                - \cus[blue]{\frac{\div \vvec{P}(\vvec{r}')}{\norm{\vvec{r}-\vvec{r}'}}}{\scriptsize -f\div \vvec{V}} \dd[3]{\vvec{r}'}
        }
        \vskip -1em
        Here we used the product rule of divergence \fbox{$\div{(f\vvec{V})} = f\div\vvec{V} + \vvec{V}\cdot\grad f$}, 
        where $f$ is a scalar function (like $\inv{\norm{\vvec{r}-\vvec{r}'}}$) and $\vvec{V}$ is a vector function (like $\vvec{P}$).
...
\begin{itemize}
    ...
    Recall that we can use sign of flux to identify converging/diverging vectors,
...
\begin{notation}[Side note:]
    ...
    \begin{itemize}
        ...
        Especially when the structure is observed far away.
    \end{itemize}
...
The general theory about material's E-field response is by
treating the polarization field $\vvec{P}$ (i.e. new dipole alignment) as some function to the total E-field $\vec{E}_\text{total}$,
\aleq{
...
The linear model only contains 1 parameter - 
...
where $\chi_e$ is called \bf{electric susceptibility}, 
a pure number (no unit) whose value depends on the type of material.
$\epsilon_0$ is multiplied to match the units. 
\begin{itemize}
    \item Vacuum has $\chi_e = 0$ by defintion, because there are no dipoles in a vacuum.
    \item Most material has $\chi_e>0$, because normally dipole moments follow E-field's direction.
\end{itemize}
...
The charges that build up in the setup is given the name \bf{free charges},
...
\begin{itemize}
    ...
    Charges distribution \cul[red]{inside} the setup.

    \item \bf{\ul{Surface free charge density}} $\sigma_f(\vvec{r})$ - 
    Charges \cul[red]{on any surfaces} of the setup.
\end{itemize}

\vskip 1ex
In real practices, parallel plate setup is the most common way to create external E-field.
...
From Gauss's law, 
\aleq{
    \epsilon_0\div \vvec{E}_\text{total}\ &=\ \quad\ \rho\,\quad =\ \qty(\mstack{\text{All}\\\text{charges}})\\[0.5ex]
...
\begin{notation}[Side Note:]
    ...
    \begin{itemize}
        ...
        \item $\vvec{P}$ may not be ``curl = 0" since the dipole arrangement can be arbituary, 
        e.g. if they arrange into some vortex-like pattern.
        Then by $\vvec{D} = \epsilon_0\vvec{E}_\text{total} + \vvec{P}$,
        there is no guarenteed that $\vvec{D}$ is ``curl = 0" either.
    \end{itemize} 
    ...
    \aleq{
        ...
        \cbox{\mstack{\vvec{P} \text{ is}\\[1ex]\text{``curl = 0"}}}
        \quad\Rightarrow\quad
        \cbox{\mstack{\vvec{D} = \epsilon_0\vvec{E}_\text{total} + \vvec{P}\\[0.5ex]\text{is also ``curl = 0"}}}
    }
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \ul{Applied voltage} - Directly connect the setup with a power source.
...
Once we have chosen a model of the material, we can connect the two measurements by:
\aleq{

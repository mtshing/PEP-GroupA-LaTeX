...
\begin{overview}
    ...
    \end{itemize}
    \begin{center}
        \red{{{(If linear dielectric is all you need, feel free to skip all vector calculus)}}}
    \end{center}
\end{overview}
...
\insertFig{instantaneous dipole in atom}
...
\begin{itemize}
    ...
    \item \ul{Covalent bond} - Bonding electrons are shared by two atoms, 
...
Rhe classical model of electric dipole is as follows:
...
As a comparison with Columb's law formula from point charge $V(\vvec{r}) = \frac{1}{4\pi\epsilon_0}\frac{q}{\norm{\vvec{r}}}$, 
potential from a dipole is decaying by $\inv{r^2}$ and is directional - 
it depends on the charges' magnitude AND the angle (dot product) between $\vvec{p}$ and $\hhat{r}$.
...
\insertFig{single dipole under external E-field, flip}
% Remember that the dipole's direction is from minus to positive
...
\insertFig{many dipoles under external E-field, not fully align}

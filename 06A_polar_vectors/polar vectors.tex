...
P.S. It is totally normal if you are confused with all the $r, \vvec{r}$ and $\hhat{r}$ symbols. 
...
\begin{enumerate}
    ...
    Therefore its definition is uglier. But most of its use cases are in pure rotation and so the Coriolis term is $0$ anyway.
    \aleq{
        \Aboxed{
...
\subsection{Review: Relative Motion}

A very naïve example to usually used to talk about relative motion:
\begin{itemize}
    \item A sees B in a car, moving at velocity $v$.
    \item B sees an object C in the same car, moving at velocity $u$. 
    \item We can conclude that the velocity of C relative to A is $u+v$. 
\end{itemize}

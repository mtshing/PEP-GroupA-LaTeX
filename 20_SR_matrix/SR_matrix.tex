...
\begin{itemize}
    \item Period $T$ = Time separation between each pulse emission.
    \item Wavelength $\lambda$ = Distance travelled of the pulse within 1 period of time.
\end{itemize}

\insertFig{wave pulse}

For light, its travelling speed $c$ is a constant to all observers. 
This makes $\lambda = cT$ and $\lambda'=cT'$. 
With a table we can see

% copy constant speed table

Therefore the observed wavelength is 
\aleq{
    \lambda' &= \gamma (1-\beta)cT = \gamma (1-\beta)\lambda \\
    &= \frac{1-\beta}{\sqrt{1-\beta^2}}\lambda \\
    &= \sqrt{\frac{1-\beta}{1+\beta}}\lambda
}

and period is
\aleq{
    T' &= \frac{\lambda'}{c}\\
    &= \sqrt{\frac{1-\beta}{1+\beta}}\frac{lambda}{c}\\
    &= \sqrt{\frac{1-\beta}{1+\beta}}\lambda
}

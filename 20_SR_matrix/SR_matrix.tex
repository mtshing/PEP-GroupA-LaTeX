...
the transformation between \bf{spacetime coordinate system}:

\begin{itemize}
    \item Every "\bf{event}" in the spacetime can be labelled with a coordinate - 
    An event happens at time $t$ at position $(x,y,z)$ is given the coordinate $(ct,x,y,z)$
    
    \item Different \bf{observers} can describe the same event using their own coordinate systems,
    leading to different expressions of the same event. 
    E.g. For the same event, 
    observer A describes it as $(ct,x,y,z)$, 
    while observer B describes it as $(ct',x',y',z')$.
    
    \iffalse
    % move to bottom of this section
    \item \bf{Lorentz transformation matrix} $\Lambda$ connects 
    the expressions of the same event in different coordinate system of observers.
    \aleq{
        \qty(\mstack{\text{Event observed by}\\\text{Observer A}})
        = \bmat{ct'\\x'\\y'\\z'}
        = \Lambda \bmat{ct\\x\\y\\z}
        = \qty(\mstack{\text{Lorentz}\\\text{Transform}}) 
            \qty(\mstack{\text{Event observed by}}\\\text{Observer B})
    }
    \fi
\end{itemize}
...
and visualize the transform with a 2D coordinate called \bf{Minkwoski space}:

\insertFig{minkwoski transform}

The transformation matrix is 
\aleq{
    \bmat{ct'//x'} = \Lambda_\red{v}\bmat{ct//x}
}
...
\begin{enumerate}
    ...
    i.e. All the formula still yield the correct results, 
    although values to be substituted becomes different for different observers.
    ...
    Speed of light is always the same for any observers. 
    (This even includes non-inertial frame observers.)
...
\begin{itemize}
    \item A always think that B is the one moving, while A himself never moves.
    This is the same for B. 

    \item If A sees B moving with velocity $\vvec{v}$, 
    B sees A moving with velocity $-\vvec{v}$.
...
\begin{enumerate}
    ...
    \item % step 0
    Suppose the matrix $\Lambda$ transforms A’s coordinate to B’s coordinate
    \aleq{
        \bmat{ct'\\x'}
        =
        \Lambda(v)
    ...
    The subscripts $v$ on $\gamma, \sigma, \mu, \lambda$ indicate that they are functions of $v$. 
    
    \item %%%%% step 1
    \bf{The inverse of $\Lambda$ must exist} -  
    we should always be able to transfrom from B's coordinate back to A’s coordinate. 
    And since B sees A moving at velocity $-\vvec{v}$,
    the inverse of $\Lambda(v)$ should have the same expression as $\Lambda$ 
    but with all $\vvec{v}$ substituted by $-\vvec{v}$
    \aleq{
        \bmat{ct\\x} 
        = \Lambda^{-1}(v) \bmat{ct'\\x'}
        &= 
        \Lambda(-v) \bmat{ct'\\x'}\\
        %
        \Lambda^{-1}(v) = \Lambda(-v)
    ...
    When A's clock passed some time $T$, A will see 
    \begin{itemize}
        \item A himself has not move.
        \item B's position has increased by $vT$ because B is moving with velocity $v$.
    \end{itemize}
    ...
    Applying Lorentz matrix will transform these coordinate to what is observed by B:
    \begin{center}
        %\begin{tabu}{|X[c]|X[c]|X[c]|}
        \hline
        ...
        \hline \rule{0pt}{4ex}
        At $t=0$ & 
        $\bmat{\mu_v & \lambda_v \\ \sigma_v & \gamma_v} 
        \bmat{0\\0}$ & 
    ...
    Note that when B looks at himself, 
    he always see himself not moving, i.e. always stay at his origin $(x'=0)$. 
    ...
    \aleq{
        ...
        \bmat{ct'\\x'}\\
        %
        \bmat{\cdots \\ \sigma_vcT+ \gamma_vvT}
        &=
        \bmat{\cdots\\0}\\
        %
        \sigma_v &= -\gamma_v\frac{v}{c}
    }
    ...
    When B's clock passed some time $T'$, B will see:
    \begin{itemize}
        \item B himself has not move.
        \item A's position has changed by $-vT'$ because A is moving with velocity $-v$.
    ...
    Applying inverse Lorentz matrix will transform these coordinate to what is observed by A:
    \begin{center}
        %\begin{tabu}{|X[c]|X[c]|X[c]|}
        \hline
        \quad & A look at A (himself) & A look at B 
        \\ \hline \rule{0pt}{4ex}
        At $t'=0$ & 
        $\bmat{\mu_{-v} & \lambda_{-v} \\ \sigma_{-v} & \gamma_{-v}}
        \bmat{0\\0}$ & 
        $\bmat{\mu_{-v} & \lambda_{-v} \\ \sigma_{-v} & \gamma_{-v}}
        \bmat{0\\0}$ 
        \\[0.5ex] \hline \rule{0pt}{4ex} 

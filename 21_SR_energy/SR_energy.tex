...
\begin{overview}
    \begin{itemize}
        \item 4-velocity and 4-momentum 
...
Plainly speaking, 
we want to combine certain physical quantities into 4$\times$1 vectors
$\bmat{\lambda & \mu & \nu & \xi}$ such that they satisfy
\aleq{
    \tkn{coorB}{\cul[red]{\bmat{\lambda'\\ \mu'\\ \nu' \\\xi'}}}
    = \tkn{lorentz}{
        \cul[gray]{\bmat{
    ...
    \tkn{coorA}{\cul[blue]{\bmat{\lambda\\ \mu\\ \nu\\\xi}}}
}
\addArrow[red]{coorB}{(-8ex,0)}
{$\lambda,\mu,\nu,\xi$   \\ using the values seen by B}
{(-3ex,0)}{(-8ex,-2ex)}
\addArrow[blue]{coorA}{(8ex,0)}
{A vector \\ made of $\lambda, \mu, \nu, \xi$ \\[1ex] \bf{Substitute the values} \\ \bf{that are observed by A}}
{(3ex,0)}{(8ex,-2ex)}
\addArrow[gray]{lorentz}{(0,-4ex)}{The Lorentz Transformation\\A 4$\times$4 matrix}{(0,-9ex)}{(0,-1.5ex)}

\vskip 3.5em

We have already seen the example of \bf{4-position vector} $\vvec{X} = \bmat{ct & x & y & z}$,
a vector made of time coordinate $t$ and position coordinate $x,y,z$ of an event,
...
we can construct other types of 4-vectors used in relativistics mechanics.


%%%%%%%%%%%%%%
\subsection{4-velocity}

This time we want to use construct a 4-vector using observed velocity $v_x$, $v_y$, $v_z$ of an object.
\aleq{
...
The problem about relativity is that 
time scale $\Delta t$ is different between observers!
If we naively "divide" a 4-position vector by the change in time of each observer, 
the Lorentz transform property is broken.
\aleq{
...
But because $\Delta t' \neq \Delta t$,
\aleq{
    \dvv{\vvec{X}'}{t'} \sim \inv{\Delta t'}\bmat{c(\Delta t')\\x'}
    \red{\neq} 
}

In order to construct a "velocity-like" 4-vector,
we must choose a unique time scale $\Delta \tau$ for differentiation such that
\aleq{
...
Which observer has the unique time scale in relativity?
For every object,
there is always a \blue{co-moving} observer, who
\begin{itemize}
    \item moves together with the object.
    \item measures the shortest time separation for any 2 stationary events on the object. 
\end{itemize}

\insertFig{3 guys: we agree to use tau as diferentiation when describing the ball}

\iffalse
I move together with the ball and my time scale is tau
you should all use tau to construct 4-velocity
OK
OK
\fi

The time scale $\Delta tau$ measured by the co-moving observer 
is called the \bf{proper time scale}. 
We can examine the values of the 4-vector after differentiation:

% table seen by A, B only, no need C
% Pos of event 1
% Pos of event 2
% Difference in coordinate 
% 4 velocity \sim 1/Delta tau \Delta x

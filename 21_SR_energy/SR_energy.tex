...
4-vectors are "vector-like objects" that
\begin{itemize}
    \item Are constructed by physical quantities (the "observables"). 
    \item Respect the rule of Lorentz transformation for vectors.
\end{itemize}

Plainly speaking, we can write the values of some physical quantities into a combination of 4 items, 
such that by multiplying with Lorentz transformation matrix, 
it yields the values of the quantities that are observed by another observer.
\aleq{
...
{A 4x1 vector\\ made of $\lambda, \mu, \nu, \xi$ \\[1ex] \bf{Substitute the values} \\ \bf{that are observed by B}}
{(-3ex,0)}{(-8ex,-2ex)}
\addArrow[blue]{coorA}{(8ex,0)}
{A 4x1 vector \\ made of $\lambda, \mu, \nu, \xi$ \\[1ex] \bf{Substitute the values} \\ \bf{that are observed by A}}
{(3ex,0)}{(8ex,-2ex)}
\addArrow[gray]{lorentz}{(0,-4ex)}{The Lorentz Transformation\\A 4x4 matrix}{(0,-9ex)}{(0,-1.5ex)}

\vskip 3.5em

We have already seen the example of \bf{4-position vector} $\vvec{X} = \bmat{ct & x & y & z}$,
which is constructed by the time and position coordinate of an event,
...
{A 4x1 vector\\ made of $t, x, y, z$ \\[1ex] \bf{Substitute the values} \\ \bf{that are observed by B}}
{(-3ex,0)}{(-8ex,-2ex)}
\addArrow[blue]{coorA2}{(8ex,0)}
{A 4x1 vector \\ made of $t, x, y, z$ \\[1ex] \bf{Substitute the values} \\ \bf{that are observed by A}}
{(3ex,0)}{(8ex,-2ex)}
...
An object is observed to be at spacetime position $\vec{X}$, it is defined as 
\[
\vec{U} = \dd{\tau} \vec{X} = \dd{\tau} \bmat ct\\x\\y\\z \emat
\]
where $\tau=$ proper time, the time scale of the observer who is relative at rest to the object. 

...
\begin{itemize}
    ...
    \aleq{
        \vvec{s} &= s_x \hhat{x} + s_y \hhat{y} \\
        \dv{\vvec{s}}{t} &= \dv{s_x}{t}\hhat{x} + s_x\red{\underbrace{\dv{\hhat{x}}{t}}_{0}} 
                            + \dv{s_y}{t}\hhat{y} + s_y\red{\underbrace{\dv{\hhat{y}}{t}}_{0}}
    }
    ...
    \aleq{
        ...
        \dv{\vvec{s}}{t} &= \dv{s_r}{t}\hhat{r} + s_r \blue{\underbrace{\dv{\hhat{r}}{t}}_{\substack{\hhat{r}\text{ contains } \\ r\text{ \& }\theta}}}
                            + \dv{s_\theta}{t}\hhat{\theta} + s_\theta \blue{\underbrace{\dv{\hhat{\theta}}{t}}_{\substack{\hhat{\theta}\text{ contains } \\ r\text{ \& }\theta}}}
    }
    \blue{$\hhat{r}, \hhat{\theta}$ are functions of $r$ \& $\theta$, and $r$, $\theta$ are functions of $t$. So differentiating against $t$ is non-zero in general.}
\end{itemize}
...
\aleq{
    \vvec{r} = r \tikzmark{rhat}\underline{\hhat{r}} \,\overset{\substack{\text{more} \\ \text{accurately}}}{=}\, r\cdot \hhat{r}_{\tikzmark{rhat2}\green{\text{at }(r,\theta)}}
}\\
...
\begin{enumerate}
    ...
    \aleq{
        \Aboxed{
            \dd{\vvec{\theta}} = (\dd{\theta})\hhat{z} = \frac{\hhat{r}_\green{\text{at }(r,\theta)}\cross\dd{\vvec{r}}}{r}
        }
    }
    ...
    \aleq{
        ...
        + r\cdot \dv{\theta}{t} \hhat{r}_\green{\text{at }(r,\theta)}\cdot \hhat{\theta}_\green{\text{at }(r,\theta)}\\[0.5em]
        &= r\dv{\theta}{t}\hhat{z}
    }
    The \red{angular velocity vector} is formally defined in $\hhat{z}$ direction:
    \aleq{
        \Aboxed{
            \vvec{\omega} = \omega \hhat{z} = \dv{\theta}{t}\hhat{z} = \frac{\hhat{r}_\green{\text{at }(r,\theta)}\cross\vvec{v}}{r}
        }
    }
...
As in the above derivations, the angular velocity is only one of the two components in the velocity vector. 
It is forbidden to do vector addition on only one of the components. 
To do it appropriately, we must also consider the radial component and the unit vectors. 

\insertFig{relative angular velocity addition is bad}

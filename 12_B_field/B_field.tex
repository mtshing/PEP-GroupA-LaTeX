...
\begin{center}
    ...
    \begin{minipage}{0.3\textwidth}
        \aleq{
            ...
            \tkn{maxwell_ampere}{\curl} \vvec{B} &= \mu_0 \vvec{J} \tkn{disp_curr}{\gray{\, +\, \mu_0\epsilon_0\pdvv{\vvec{E}}{t}}}
        }
    \end{minipage}
\end{center}
\addArrow[blue]{maxwell_div}{(-5ex,0)}{}{(-3ex,1ex)}
\addArrow[red]{maxwell_ampere}{(-5ex,0)}{}{(-3ex,1ex)}
\addArrow[gray]{disp_curr}{(6ex,0)}
{\scriptsize The second term \\[-1ex]\scriptsize is unrelated \\[-1ex]\scriptsize in "statics" case}
{(6.5ex,0.5ex)}{(4.5ex,0)}
...
- \cul[blue]{Gauss's law on B-field} and \cul[red]{Ampere's law} (not including the second term). 
...
% content begins here
% Section %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Biot-Savat Law with Integration}
...
but can flow on a surface or through an object.
In these cases, the current flow varies at different position
and should be described as a distribution of flow (i.e. a vector field).
\begin{center}
    \vskip 1em
    \begin{minipage}{0.45\linewidth}
        \centering
        When current is travelling on a surface, 
        the current flow is a 2D vector field, 
        called \bf{surface current density} $\vvec{K} = \vvec{K}(x,y)$. 
        \aleq{
            I\dd{\vvec{l}} 
    ...
    \begin{minipage}{0.45\linewidth}
        ...
        the current flow is a 3D vector field, 
        called \bf{volume current density} $\vvec{J} = \vvec{J}(x,y,z)$.
        \aleq{
            I\dd{\vvec{l}} 
...
\vskip 1ex
\ul{Note:} By charge conservation,
current along a line wire must be constant (flow in = flow out).
\phantom{\ul{Note:}} Therefore it is rare to refer $I$ as the \ul{line current density}. {\scriptsize (Just call it current.)}
...
\aleq{
    ...
    %
    = &\qty(\frac{F_y\qty(x+\frac{\Delta x}{2},y,z) - F_y\qty(x-\frac{\Delta x}{2},y,z)}{\blue{\Delta x}}
        - \frac{F_x\qty(x,y+\frac{\Delta y}{2},z) - F_x\qty(x,y-\frac{\Delta y}{2},z)}{\blue{\Delta y}}) (\Delta x\Delta y)\\[1em]
    %
    = &\qty(\cut[red]{\cbox[red]{\frac{F_y\qty(\cbox[red]{\textstyle x+\frac{\Delta x}{2}},y,z) 
            - F_y\qty(\cbox[red]{\textstyle x-\frac{\Delta x}{2}},y,z)}{\cbox[red]{\Delta x}}}}{\text{This is exactly partial x}}
...
\aleq{
    ...
    \iint (\tkn{curlB}{\curl \vvec{B}}) \cdot \dd{\vvec{s}}\ &=\ \mu_0\iint \tkn{curlI}{\vvec{J}} \cdot \dd{\vvec{s}}
}
...
\aleq{
    ...
    &= \tkn{ampere_B}{\cul[red]{\norm{\vvec{B}}}}\ \tkn{ampere_theta}{\cul[blue]{\cos\theta}}\ \oint \norm{\dd{\vvec{l}}}\\[2.5em]
...
\begin{notation}[Side note:]
    ...
    \end{itemize}
    Here are the \nth{2} derivatives relations of $\grad$ (can be proved by brute force):
    \begin{itemize}
        \item \ul{Finish off by gradient}: \\[-3ex]
        \aleq{
...
\begin{notation}[]
    \begin{itemize}
        \item \ul{Finish off by divergence}:
        \aleq{
        ...
        \item \ul{Finish off by curl}:
        \aleq{
            \curl (\grad f) &\ \equiv\ 0 \qquad \red{\scriptsize \text{(Curl of gradient is always 0)}}\\[1ex]
            %
            \curl (\curl \vvec{F}) &\ \equiv\ \ \grad (\div \vvec{F}) - \laplacian \vvec{F} \qquad {\scriptsize \text{(This is a useful identity)}}
        }
    \end{itemize} 
